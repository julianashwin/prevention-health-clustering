...
`04_build_grm.py` refits the EIT note's graded-response model with the
package's own estimator and gates on the August 2026 reference fit: exact
calibration sample (498,424 person-years), discriminations to 3e-5, and —
with `--reference-dir` pointing at CSVs extracted from `grm.rds` —
per-person-wave theta to 1e-5 and the age latent distributions to 5dp.
