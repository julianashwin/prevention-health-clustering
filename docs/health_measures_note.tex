...
scale, with the implied measurement SE (dashed). Panels, left to right: the
original four-testlet physical GRM (GH, PF, RP, BP); P-FUNC (those four plus
FUNC); P-FULL (P-FUNC plus the six diagnosis groups); MENT, the mental bank
(MH, RE, SF, the two GHQ wording testlets, DEPR); and COMBINED, all
seventeen items. FUNC adds a broad band of precision on the ill side; the
condition items are visibly thin; GHQ-negative dominates the mental bank;
the combined bank is tallest and widest but its SE still rises sharply above
$\theta \approx +1$.}
\label{fig:info}
\end{figure}
...
20--90). Six measures: the UKHLS PCS, the physical-only subscale composite,
SF-6D utility, the original four-testlet GRM, the physical GRM in its P-FULL
specification, and the mental GRM.}
\label{fig:clusters}
\end{figure}
...
Figure 30, rebuilt): mean age path, fanning, top-censoring, and the two
contamination reads. Seven measures: UKHLS PCS, SF-6D utility, the original
four-testlet GRM, P-FUNC, P-FULL, the mental GRM and the combined GRM.}
\label{fig:versions}
\end{figure}
...
quadratic (convexity) term for in-patient admission. All thirteen measures
are tested (Table above); the seven plotted are UKHLS PCS, SF-6D utility,
the original GRM, P-FUNC, P-FULL, the mental GRM and the combined GRM, each
oriented so higher means better health.}
\label{fig:convexity}
\end{figure}
