...
re-scaling. The second is \emph{not} clean and should not be read as one:
P-FULL drops the BHPS entrants who never receive the condition inventory
(\S\ref{sec:chronic}), so its lower noise is confounded with a different
sample. It is consistent with the diagnosis items anchoring the score ---
conditions are stable and reported consistently --- but refitting P-FUNC on
the P-FULL roster is what would establish it.
