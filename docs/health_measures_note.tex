...
\textbf{What the model's held-out density actually conditions on.} The Stan
generated quantities restart the AR(1) recursion at the first held-out row
with the \emph{stationary} variance, so the person's own last fitted value
is not carried forward through $\rho$. The fitted rows reach the held-out
...
is capable of. Both are therefore reported below, computed offline on the
same rows; the offline machinery reproduces the model's own stored held-out
densities to a correlation of $0.9999998$, which validates it. The
...
\textbf{Implication for the model code.} If held-out prediction is a target
rather than a diagnostic, the generated-quantities block should carry the
last fitted value into the held-out window rather than restarting at the
stationary variance --- on these fits that is worth about $.04$--$.13$ nats
per observation and $.04$--$.09$ of $R^2$. Until then, held-out numbers
reported by the fits themselves should be read as class-structure
diagnostics, not as the model's predictive ceiling.
