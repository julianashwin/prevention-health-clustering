...
\textbf{Held-out predictive densities.} Because the channels differ in
scale, raw log densities are not comparable; each is therefore benchmarked
on its own held-out rows against an age-quadratic Gaussian null estimated on
that fit's own fitted rows.

\begin{center}\small
\begin{tabular}{lrrrr}
\toprule
Channel & model & $N(0,1)$ null & age-quadratic null & gain \\
\midrule
PCS & $-0.988$ & $-1.519$ & $-1.416$ & $+0.428$ \\
GRM phys (P-FULL) & $-0.915$ & $-1.469$ & $-1.376$ & $+0.461$ \\
GRM combined & $-0.982$ & $-1.481$ & $-1.452$ & $+0.470$ \\
\bottomrule
\end{tabular}

{\scriptsize Log predictive density per held-out observation (two per
person, 58--79k rows).}
\end{center}

All three mixtures buy $0.43$--$0.47$ nats per held-out observation over the
age-only null --- substantial, and remarkably similar across instruments.
The physical GRM has both the best absolute density and (with the combined
bank) the largest gain, but the differences between channels are small
relative to what all three gain over the null: the latent-class structure,
not the choice among these three measures, is what carries the predictive
content. Note these are \emph{within}-person predictions with the class
posterior conditioned on each person's own earlier observations --- a
different and easier estimand than the new-person scoring of
\code{scoring/new\_person.py}, and not to be compared with it.
