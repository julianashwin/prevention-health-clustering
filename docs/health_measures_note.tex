...
\emph{measurement error} on top (\S\ref{sec:meas}). Class 1 is worst health
throughout (the anchor convention). All eleven fits converged; the first
batch of eight took 15.9 h and the three state-space fits 14.9 h, both on two
workers.
...
than imposed. It is also, as \S\ref{sec:meas} shows, a statement about an
...
``ME'' in the last three titles marks the state-space specification of
\S\ref{sec:meas}: an AR(1) latent state plus an independent one-period
...
fanning in the three state-space fits (\S\ref{sec:meas}).
...
None of this is a substitute for the direct test, which is to refit P-FUNC on
the P-FULL roster; that fit has not been run. But the difference is a clean
person-level selection on survey entry rather than on health or on panel
length, and the incidental differences push the wrong way to manufacture the
result. The reading that the diagnosis items anchor the score --- conditions
are stable and reported consistently --- survives the check it could most
easily have failed.
...
the AR(1) specification. The state-space fits were run without holdout, so
whether separating the noise actually improves out-of-sample prediction is
not yet established; that is the obvious next fit.
...
\code{descriptives/01--17} & measure panel, correlations, every figure, the
...
\code{data\_cleaning/09\_build\_cost\_proxy.py},
\code{descriptives/15\_cost\_validation.py} & the cost proxy of
